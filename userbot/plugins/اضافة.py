...
async def get_chatinfo(event):
    ...
    try:
        ...
        try:
            ...
            await event.reply("**▾∮ لم يتم العثور على القروب او القناه**")
            return None
...
# كتابة فريق خليج ثون @zVVVn
# 𝗧𝗲𝗹𝗲𝗴𝗿𝗮𝗺 ~ @YDDDE ~ @zVVVn
...
async def get_users(event):
    ...
    if not sender.id == me.id:
        roz = await event.reply("**▾∮ تتـم العـملية انتظر قليلا  ...**")
    else:
        roz = await event.edit("**▾∮ تتـم العـملية انتظر قليلا  ...**.")
    ...
    async for user in event.client.iter_participants(JMTHON.full_chat.id):
        try:
            if error.startswith("Too"):
                return (
                    await roz.edit(
                        f"**حـالة الأضـافة انتـهت مـع الأخـطاء**\n- (**ربـما هـنالك ضغـط عـلى الامر حاول مججـدا لاحقـا **) \n**الـخطأ** : \n`{error}`\n\n• اضالـة `{s}` \n• خـطأ بأضافـة `{f}`"
                    ),
                )
            await event.client(
