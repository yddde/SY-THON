...
DEFAULTUSERBIO = str(DEFAULT_BIO) if DEFAULT_BIO else "الـحمد لله عـلى كـل شـيء || @zVVVn"
...
async def _(event):
    "لأرجـاع الحسـاب الـى وضعه الأصـلي"
    name = f"{DEFAULTUSER}"
    blank = ""
    ...
    await edit_delete(event, "⌯︙تـم اعاده الـحساب بـنجاح ✅")
    if BOTLOG:
        await event.client.send_message(
            BOTLOG_CHATID, f"⌯︙تـم اعاده الـحساب الى وضـعه الاصلـي ✅"
        )
