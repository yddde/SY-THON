...
@jmthon.ar_cmd(
    pattern="(ت(ل)?ق(راف)?) ?(m|t|ميديا|نص)(?:\s|$)([\s\S]*)",
    command=("تلقراف", plugin_category),
    info={
...
async def _(event):
    ...
    if BOTLOG:
        await event.client.send_message(
            BOTLOG_CHATID,
            f" ⌔︙انـشاء حسـاب تلقراف جـديد {auth_url} لـهذه الجلـسة. \n**لا تعـطي هـذا الرابـط لاي احـد, حـتى لو قـالو انـهم مـن شـركة التلقرام!**",
        )
    optional_title = event.pattern_match.group(5)
    if not event.reply_to_msg_id:
        return await jmevent.edit(
            "` ⌔︙قـم بالـرد عـلى هـذه الرسـالة للحـصول عـلى رابـط تلقراف فـورا`",
        )
