...
async def print_changelogs(event, ac_br, changelog):
    changelog_str = (
        f"**⌯︙قام مطورين السورس بتحديث خليج ثون**\n⌯︙**التـغييرات\n** {changelog}"
    )
...
async def update(event, repo, ups_rem, ac_br):
    ...
    jasme = await event.edit(
        "** ⌯︙تم تحديث سورس خليج ثون بنجاح انتظر قليلا سوف نخبرك بعد اعادة التشغيل !**"
    )
...
async def upstream(event):
    ...
    event = await edit_or_reply(
        event, "**⌯︙يـتـم البـحـث عـن تـحديثـات سـورس خليج ثون انـتـظـر**"
    )
    ...
    if changelog == "" and not force_update:
        await event.edit(
            "**⌯︙سورس خليج ثون محدث الى اخر اصدار **\n"
            f"**قـنـاة سـورس خليج ثون** : @zVVVn"
        )
        return repo.__del__()
    if conf == "" and not force_update:
        await print_changelogs(event, ac_br, changelog)
        await event.delete()
        return await event.respond(f"⌔ :  لتحديث سورس خليج ثون ارسل : `.تحديث الان` ")
    ...
    if conf == "الان":
        await event.edit("** ⌯︙جار تحـديـث سـورس خليج ثون انـتـظـر قـليـلا 🔨**")
