# @YDDDE  - @zvvvn  - @xfffw
# Copyright (C) 2021 SY-THON TEAM
# FILES WRITTEN BY  @YDDDE
import asyncio
import io
import os
import pathlib
...
async def ytdl_down(event, opts, url):
    try:
        ...
        await event.edit(
            "⌯︙- الفيديو غير متاح من موقعك الجغرافي بسبب القيود الجغرافيه التي يفرضها موقع الويب ❕"
        )
        return None
        ...
        await event.edit("⌯︙كان هناك خطأ أثناء المعالجه")
        return None
...
async def kakashi(event):
    ...
    if "www.instagram.com" not in link:
        await edit_or_reply(event, "⌯︙- يجب كتابة رابط من الانستقرام لتحميله ❕")
    else:
        start = datetime.now()
