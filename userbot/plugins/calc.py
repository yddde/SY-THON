#  قبل لا تفكر تخمط هذا الملف ترا الملف متعوب عليه لا تخمط وانا حذرتك
# حسب قوانين موقع github https://github.com/YDDDE/SY-THON/blob/master/LICENSE
# تنص على انه اي شخص ياخذ الملف بدون ذكر حقوق طبع والنشر سيتم حذف حسابه من قبل صاحب الملف اقتضى التنوي
# Copyright ©️ 2021 YDDDE . All Rights Reserved
# You are free to use this code in any of your project, but you MUST include the following in your README.md (Copy & paste)
# ##Credits -  (  @YDDDE  - @zVVVn  )


# 𝗧𝗲𝗹𝗲𝗚𝗿𝗮𝗠 : @zVVVn  ~ @YDDDEimport re
# 𝗧𝗲𝗹𝗲𝗚𝗿𝗮𝗠 : @zVVVn  ~ @YDDDE
...
@jmthon.ar_cmd(
    ...
    info={
        "header": "حاسبه علميه",
        "usage": "{tr}حاسبه",
        "description": "امر حاسبه بوضع الانلاين مميزة ورائعة.",
    },
)
async def icalc(e):
    if e.client._bot:
        return await e.reply(
            "**الحاسبه العلميه لسـورس خليج ثون\n @zVVVn**", buttons=lst
        )
...
async def inlinecalc(event):
    ...
    if (
        ...
        calc = event.builder.article(
            "Calc", text="**الحاسبه العلميه لسـورس خليج ثون\n @zVVVn**", buttons=lst
        )
...
async def _(e):
    ...
    lst = list(zip(tultd[::4], tultd[1::4], tultd[2::4], tultd[3::4]))
    lst.append([Button.inline("=", data="calc=")])
    await e.edit("**الحاسبه العلميه لسـورس خليج ثون\n @zVVVn**", buttons=lst)
