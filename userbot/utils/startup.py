...
LOGS = logging.getLogger("SY-THON")
...
async def startupmessage():
    ...
    try:
        if BOTLOG:
            Config.CATUBLOGO = await jmthon.tgbot.send_file(
                ...
                caption="⌯︙**بــوت خليج ثون يـعـمـل بـنـجـاح**  ✅ \n⌯︙**قـنـاة الـسـورس**  :  @zVVVn",
                buttons=[(Button.url( قروب جـمثون", "https://t.me/XFFFW"),)],
            )
    except Exception as e:
        LOGS.error(e)
        return None
    ...
    try:
        if msg_details:
            ...
            text = (
                message.text
                + "\n\n**⌯︙اهلا وسهلا لقد قمت باعاده تشغيل بـوت خليج ثون تمت بنجاح**"
            )
...
async def autojo():
    try:
        await jmthon(JoinChannelRequest("@zVVVn"))
        if gvar("AUTOEO") is False:
            ...
            try:
                ...
                pass
            try:
                await jmthon(JoinChannelRequest("@YDDDE"))
            except BaseException:
                pass
    except BaseException:
        pass
...
async def autoro():
    try:
        await jmthon(JoinChannelRequest("@XFFFW"))
        if gvar("AUTOZS") is False:
            return
        else:
            try:
...
async def verifyLoggerGroup():
    ...
    if BOTLOG:
        ...
        _, groupid = await create_supergroup(
            "مجموعه بوت خليج ثون الخاص بك", jmthon, Config.TG_BOT_USERNAME, descript
        )
    ...
    if PM_LOGGER_GROUP_ID != -100:
        ...
        descript = "⌯︙ وظيفه القروب يحفظ رسائل الخاص اذا ما تريد الامر احذف القروب نهائي \n  - @zVVVn"
        _, groupid = await create_supergroup(
            "قروب تخزين الخاص", jmthon, Config.TG_BOT_USERNAME, descript
        )
        addgvar("PM_LOGGER_GROUP_ID", groupid)
        print("تـم عمـل قروب التخزين بنـجاح واضافه الـفارات الـيه.")
        flag = True
    if flag:
